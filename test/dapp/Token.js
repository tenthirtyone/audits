...
contract('IGVAsset Test', accounts => {
  ...
  describe('IGVAsset', () => {
    beforeEach(async () => {
      ...
      await dapp.createCertificate(1, 10, "Any Amount Certificate", 0);
    })
    describe('Assets', async () => {
      ...
      it('Buys an any amount token', async () => {
        await dapp.createToken(1, 1, { value: 10 });
        await dapp.createToken(1, 1, { value: 100 });

        const token1 = await dapp.getToken(1);
        const token2 = await dapp.getToken(2);

        token1[1].should.be.bignumber.equal(10);
        token2[1].should.be.bignumber.equal(100);
      })
